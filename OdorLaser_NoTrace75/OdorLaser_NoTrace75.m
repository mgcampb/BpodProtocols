...
% M. Campbell 5/5/2026: Trace to zero seconds, 100% stim probability, no
%   rewards, no unpredicted opto-stim, 8 trial block for
...
% Make sure the first TrialType1 trials are not omission trials
ctr = 1;
for i = 1:ActualChunkSize
    if TrialTypes(i)==1
        if ctr<=round(ActualChunkSize*S.StimProbability/S.NumOdors)
            StimTrials(i) = 1;
        else
            StimTrials(i) = 0;
        end
...
for currentTrial = 1:NumOdorTrials
    ...
    sma = AddState(sma, 'Name', 'Laser',...
        ...
        'OutputActions', {'ValveModule1', 1,'WavePlayer1', LaserMessage, 'BNC1', 0, 'BNC2', 0});
