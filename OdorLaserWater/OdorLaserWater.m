...
TargetChunkSize = 50; % trials; chunk size in which to balance trial types
...
RewardedTrialsChunk = [ones(1,round(ActualChunkSize * S.RewardProbability)) ...
    zeros(1,round(ActualChunkSize * (1-S.RewardProbability)))];
RewardedTrialsChunk = RewardedTrialsChunk(1:ActualChunkSize);
TrialTypes = [];
RewardedTrials = [];
for i = 1:NumChunks
    perm_idx = randperm(ActualChunkSize);
    TrialTypes = [TrialTypes TrialTypesChunk(perm_idx)];
    RewardedTrials = [RewardedTrials RewardedTrialsChunk(perm_idx)];
end
TrialTypes = TrialTypes(1:MaxTrials);
RewardedTrials = RewardedTrials(1:MaxTrials);
RewardedTrials(TrialTypes==max(TrialTypes)) = 0; % the last trial type is nothing odor
