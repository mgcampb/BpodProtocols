...
FR_func_SuperExpRamp = @(lambda)((1/lambda).^((t_exp/t_end).^2)-1/lambda+FR_max);

lambda_min_all = linspace(0.033, 0.0393, 6); % [0.033 0.038 0.041 0.042 0.045 0.048];
...
for i = 1:numel(gamma)
    ...
    target_exp = FR_func_expRamp(t_exp, t_end, gamma(i), FR_min, FR_max);
    % lambda_min = fmincon(@(lambda)(sum((FR_func_SuperExpRamp(lambda)-target_exp).^2)),3,[],[],[],[],0.000001,0.999999);
    lambda_min = lambda_min_all(i);
    target_superExp = FR_func_SuperExpRamp(lambda_min);

    figure; hold on;

    plot(t_exp,target_exp);
    plot(t_exp,target_superExp);

    target_superExp = [target_superExp FR_max*ones(1,SR*buffer_t)];
    target_superExp = fliplr(target_superExp);
    waveform = PulseTrain(target_superExp, t_tot, PulseDur);
