...
FR_func_quadraticRamp = @(alpha)(alpha*t_exp.^2+((FR_max-FR_min)/t_end-alpha*t_end)*t_exp+FR_min);
...
for i = 1:numel(gamma)
    ...
    target_exp = FR_func_expRamp(t_exp, t_end, gamma(i), FR_min, FR_max);
    alpha_min = fmincon(@(alpha)(sum((FR_func_quadraticRamp(alpha)-target_exp).^2)),0.5,[],[],[],[],0,10);
    target_quad = FR_func_quadraticRamp(alpha_min);

    figure; hold on;

    plot(t_exp,target_exp);
    plot(t_exp,target_quad);

    target_quad = [target_quad FR_max*ones(1,SR*buffer_t)];
    target_quad = fliplr(target_quad);
    waveform = PulseTrain(target_quad, t_tot, PulseDur);
...
save('stimWaveforms_quadraticRamp.mat','stimWaveforms');
