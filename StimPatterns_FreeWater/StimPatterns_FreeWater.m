...
% 1) 1 sec at 20 Hz
waveform = zeros(1,round(SR/20));
waveform(1:(0.005 * SR)) = 5;
waveform = repmat(waveform,1,20);
...
for i = 1:24
    ...
    waveform = [waveform 5*ones(1,numOnes) zeros(1,numZeros)];
    freq = freq*1.135; 
end
W.loadWaveform(3,waveform);
WavePlayerMessages = [WavePlayerMessages {['P' 1 2]}];
% subplot(4,1,3); plot((0:(numel(waveform)-1))/SR,waveform); xlim([0 2]); xlabel('sec'); ylabel('V'); title('Pattern 3');
...
W.loadWaveform(4,waveform);
WavePlayerMessages = [WavePlayerMessages {['P' 1 3]}];
% subplot(4,1,4); plot((0:(numel(waveform)-1))/SR,waveform); xlim([0 2]); xlabel('sec'); ylabel('V'); title('Pattern 4');
