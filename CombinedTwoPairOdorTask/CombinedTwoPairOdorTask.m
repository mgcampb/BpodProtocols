function CombinedTwoPairOdorTask
    ...
    % C. Chen 2/3/2026: Changed to 8 odors
    
    % C. Chen 2/5/2026: Changed from 5 trials per odor per block to 6
    ...
    if isempty(fieldnames(S))
        ...
        S.NumOdors = input('Number of odors: '); % 8 
    ...
    S.NumOdorTrials = OdorChunkSize*3; % 99 trials in total, 3 free reward trials, 123 total trials
    
    S.RewardDelay = [0.25 0.25 1 1 2.5 2.5 5.5 5.5]; % one per odor
    
    % 5 trials per odor per chunk
    S.FracTrials_Odor = repmat(6/OdorChunkSize, 1, S.NumOdors);  % 8 elements, each 5/41
    S.FracTrials_Free = 1 - sum(S.FracTrials_Odor);             % = 1/41 (exact)
