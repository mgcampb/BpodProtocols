function StimPatterns_FreeWater_MultiSite_Burst
...
% M. Campbell 8/14/24 - Burst - 1,2,3.. pulses at high frequency (to
...
% make them all LED2 (MGC 8/14/24 - sharing rig with Simon)
TrialTypes = mod(TrialTypes,10)+20;
...
% 1) One pulse
waveform_OnePulse = zeros(1,round(SR/S.PulseFreq));
waveform_OnePulse(1:(S.PulseDur_sec * SR)) = 5;
W.loadWaveform(1,waveform_OnePulse);


% 1) Two pulses
waveform_TwoPulse = [waveform_OnePulse waveform_OnePulse];
W.loadWaveform(2,waveform_TwoPulse);


% 1) Three pulses
waveform_ThreePulse = [waveform_TwoPulse waveform_OnePulse];
W.loadWaveform(3,waveform_ThreePulse);

% save waveforms:
S.stimWaveforms = {waveform_OnePulse, waveform_TwoPulse, waveform_ThreePulse};
