...
FR_func_SuperExpRamp = @(lambda)((FR_max-FR_min)*exp((t_end-t_exp).^2*log(lambda))+FR_min);
...
for i = 1:numel(gamma)
    ...
    target_exp = FR_func_expRamp(t_exp, t_end, gamma(i), FR_min, FR_max);
    lambda_min = fmincon(@(lambda)(sum((FR_func_SuperExpRamp(lambda)-target_exp).^2)),3,[],[],[],[],0.00001,0.99999);
